...
fn language(path: &Path) -> Option<String> {
    match path
        .extension()
        .and_then(|s| s.to_str())
        .map(|s| s.to_ascii_lowercase())
    {
        Some(ext) if ext == "py" || ext == "pyw" => Some("python".into()),
        _ => None,
    }
}
...
impl ProjectScanner {
    pub fn scan(&self, root: &Path) -> Result<Vec<DiscoveredFile>> {
        ...
        let mut out = Vec::new();
        builder.filter_entry(|e| !excluded(e));
        for item in builder.build() {
            ...
            out.push(DiscoveredFile {
                ...
                hash: hash(&bytes).to_hex().to_string(),
...
mod tests {
    use super::*;
    #[test]
    fn deliberately_narrow() {
        assert_eq!(language(Path::new("a.py")).as_deref(), Some("python"));
        assert_eq!(language(Path::new("a.js")), None);
    }
}
