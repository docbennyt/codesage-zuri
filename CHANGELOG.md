...
- Replaced repository-global same-name call guessing with lexical/same-file and explicit import-aware Python resolution.
- Added imported-alias and module-qualified repository call resolution.
- Added separate `Resolved`, `Probable` and `Unresolved` call states; `self` / `cls` method links remain probable rather than authoritative.
- Added Evidence Contract labels: `FACT`, `DOCUMENTED`, `INFERENCE`, `MODEL` and prevented probable call edges from entering model-facing FACT evidence.
...
- Added `zuri benchmark` for local cold/incremental indexing, symbol lookup, Knowledge Pack query and index-size measurements, plus Linux process high-water RSS.
